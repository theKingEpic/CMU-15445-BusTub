...
auto HashJoinExecutor::Next(Tuple *tuple, RID *rid) -> bool {
  ...
  while (true) {
    // 如果right_tuple_不为空（Init)，且jht_iterator_未遍历完，则说明找到了匹配的元组。
    // 在这种情况下，遍历right_tuple_的迭代器jht_iterator_，
    // 并使用left_tuple_.GetValue()和right_tuple.GetValue()方法获取左右元组的值，
    // 将它们组合成一个新的Tuple对象，并返回true。
    // 一个左边可能匹配多个右边
    if (right_tuple_ != nullptr && jht_iterator_ != right_tuple_->end()) {
      std::vector<Value> values;
      // right_tuple_是vector，而Next一次只输出一个Tuple，所以这里用jht_iterator_迭代，right_tuple是单个Tuple
      auto right_tuple = *jht_iterator_;
      // this->left_child_->GetOutputSchema().GetColumnCount()指的是左表有多少列！
