...
DiskManager::DiskManager(const std::string &db_file) : file_name_(db_file) {
  ...
  /*
   * 在C++中，std::string::npos是一个特殊的常量，它被定义为std::string类的一个静态成员，用于表示不存在的位置。
   * 当你在一个字符串中搜索一个子串或字符，并且没有找到它时，函数通常会返回npos。

    std::string::npos的值是一个非常大的正整数，通常是-1（0xFFFFFFFF），因为在std::string内部，
    位置通常是用一个无符号整数来表示的。这个值被保证大于任何有效字符串的长度，因此可以安全地用作指示搜索失败的标志。
   */
...
void DiskManager::ReadPage(page_id_t page_id, char *page_data) {
  ...
  if (offset > GetFileSize(file_name_)) {
    ...
    if (read_count < BUSTUB_PAGE_SIZE) {
      ...
      // 这行代码检查实际读取的字节数是否少于一个页面的大小。如果是，清除I/O流的错误状态，并将剩余的部分填充为0。
