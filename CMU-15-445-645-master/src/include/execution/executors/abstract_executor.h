 ...
 * AbstractExecutor实现了Volcano一次元迭代器模型。
   这是BustTub执行引擎中所有执行器继承的基类，并定义了所有执行器支持的最小接口。
 */
class AbstractExecutor {
 public:
  /**
   * Construct a new AbstractExecutor instance.
   * @param exec_ctx the executor context that the executor runs with执行程序运行时使用的执行程序上下文
   */
