...
struct HashJoinKey {
  ...
  /**
   * Compares two hash joi keys for equality比较两个散列键是否相等
...
class SimpleHashJoinHashTable {
 public:
   ...
   * 如果哈希表中没有join_key，则创建一个新的std::vector来存储与该键相关的所有Tuple对象，
   * 并将当前的tuple添加到该vector中。如果哈希表中已经存在join_key，则直接将tuple添加到已有的vector中。
   * */
...
class HashJoinExecutor : public AbstractExecutor {
 ...
 private:
  /*    HashJoinKey的值是用于连接的键值，它们是从左子执行器的元组中提取的。存储的是原始值
  哈希值是基于HashJoinKey中的键值计算得出的，用于快速定位哈希表中的键值对。*/
  /*从左子执行器产生的元组中提取连接条件中的值，并将它们组合成一个HashJoinKey对象。
   * 这个HashJoinKey对象将用于在哈希表中索引右子执行器的元组。*/
