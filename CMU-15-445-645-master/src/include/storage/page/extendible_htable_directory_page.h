...
class ExtendibleHTableDirectoryPage {
 public:
  ...
  /**
   * GetGlobalDepthMask - 返回一个全局深度掩码，该掩码由全局深度个1和其余的0组成。
   *
   * 在可扩展哈希中，我们使用哈希+掩码函数将键映射到目录索引。
   *
   * DirectoryIndex = Hash(key) & GLOBAL_DEPTH_MASK
   *
   * 其中GLOBAL_DEPTH_MASK是一个掩码，从最低有效位（LSB）开始正好有GLOBAL_DEPTH个1。
   * 例如，全局深度为3在32位表示中对应于0x00000007。
   *
   * @return 一个全局深度个1和其余0组成的掩码（从最低有效位开始为1）。
 ...
 private:
  ...
  uint8_t local_depths_[HTABLE_DIRECTORY_ARRAY_SIZE];       //一个桶局部深度的数组
  page_id_t bucket_page_ids_[HTABLE_DIRECTORY_ARRAY_SIZE];  //包含第三层的BucketPage的PageId数组
};
