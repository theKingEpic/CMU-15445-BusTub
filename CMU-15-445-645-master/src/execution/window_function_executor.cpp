...
auto WindowFunctionExecutor::MakeWinKey(const Tuple *pTuple, std::vector<AbstractExpressionRef> &partition_by) const {
  AggregateKey key;
  // 计算每个分组表达式的值，并存储在AggregateKey的group_bys_中
  for (const auto &expr : partition_by) {
    Value value = expr->Evaluate(pTuple, this->GetOutputSchema());
    key.group_bys_.push_back(value);
  }
  return key;
}

auto WindowFunctionExecutor::MakeWinValue(const Tuple *pTuple, std::shared_ptr<AbstractExpression> &expr) const {
  // 计算表达式在元组上的值
  Value value = expr->Evaluate(pTuple, this->GetOutputSchema());
  return value;
}
...
void WindowFunctionExecutor::Init() {
  ...
  for (uint32_t i = 0; i < cloumn_size; i++) {
    ...
    if (window_functions.find(i) == window_functions.end()) {
      ...
      // 没有排序
      is_order_by[i] = false;
      // 将空的窗口函数类型也存入SimpleWindowHashTable的vector中，方便后续遍历使用
